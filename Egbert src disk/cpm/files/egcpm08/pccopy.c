...
*									      *
*******************************************************************************
* folgende Files muessen mit gelinkt werden:                                  *
* WINDOW.C | CHECKTRK.MAC | FORMTRK.MAC | PCCOPY.C                            *
* Bibliothek: NEWLIBC.REL | ein hartes Stueck try and error; Egbert Schr|er   *
* 27.12.92                                                                    *
******************************************************************************/
      ...
      switch (mode = *s - '0')
      {
        case 1 : /* get source drive */
		 ...
		 while (entry)
		 {
		   ...
		   {
		     /* clear old clusters of file */
		     oldcl = pcentry[i].firstcl;
  ...
  int cluster;
{
  ...
  if (cluster >= clcount && cluster < 0xFF7)
  {
    printf   ("\nDisk full\n");
    deselect ();
    xabort   (2, drive);
