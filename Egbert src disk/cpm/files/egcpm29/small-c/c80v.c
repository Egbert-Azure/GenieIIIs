...
#define VERSION "27.November 1986"
#include iolib.h
#include float.h

#define BANNER  "**** C-Compiler Small-C V1.2D ****"
#define AUTHOR "Autoren: Ron Cain und James Van Zandt"
...
/*	dumplits();		deleted 6/21/82  jrvz */
	...
	exit(); /* return removed, mk 28 nov 86 */ /* then exit to system */
	}
...
abort()
{
	...
	closeout();
	toconsole();
	pl("Compilierung abgebrochen");  nl();
	exit();
/* end abort */}
...
errsummary()
	...
	if (ncmp) error("Fehlende rechte Klammer");
		/* open compound statement ... */
	nl();
	outstr("Es wurden ");
	outdec(errcnt);	/* total # errors */
	outstr(" Fehler gefunden.");
	nl();
	}
...
{	char *count,	/* *count is # characters in command line */
	...
	*next;	/* where the next byte goes in args[] */
	...
	pl(AUTHOR);
	nl();
	pl("Uebersetzung & Aenderungen: (C) Martin Kotulla");
	nl();
	pl(VERSION);
	nl();
	nl();
...
openout()
	...
	if((output=fopen(line,"w"))==NULL) /* if given, open */
		{output=0;	/* can't open */
		error("Ausgabedatei kann nicht geoeffnet werden");
		}
	kill();			/* erase line */
}
...
openin()
{
	...
	while(input==0){	/* any above 1 allowed */
		...
		if((input=fopen(line,"r"))!=NULL)
			...
			pl("Datei kann nicht geoeffnet werden ");
			}
		}
	kill();		/* erase line */
	}
...
doinclude()
{
	...
	if(inpt2)					/* gtf 7/16/80 */
		error("Verschachtelte INCLUDEs nicht zulaessig");
	else if((inpt2=fopen(line+lptr,"r"))==NULL)
		{inpt2=0;
		error("INCLUDE-Datei nicht gefunden");
		}
...
needsub()
	...
	if (number(num)==0)
			/* go after a number */
		{error("Muss eine Konstante sein");	/* it isn't */
		num[0]=1;		/* so force one */
		}
	if (num[0]<0)
		{error("Unzulaessiger negativer Wert");
		num[0]=(-num[0]);
		}
...
newfunc()
	...
	*iptr;  /* temporary ptr for stepping along argument
					chain jrvz 8/6/82 */
	if (symname(n)==0)
		{error("Unzulaessige Funktion oder Deklaration");
	...
	if(match("(")==0)error("Fehlende oeffnende Klammer");
	...
	while(match(")")==0)	/* then count args */
		...
		else{error("Unzulaessiger Name des Arguments");junk();}
		blanks();
		/* if not closing paren, should be comma */
		if(streq(line+lptr,")")==0)
			{if(match(",")==0)
			error("Komma erwartet");
			}
		if(endst())break;
		}
	...
	while(undeclared)
		...
		else{error("Falsche Zahl von Argumenten");break;}
		}
...
getarg(t)	/* t = cchar, cint or DOUBLE jrvz 8/5/82 */
	...
	while(undeclared)  /* jrvz 8/6/82 */
		...
		if(legalname)
			...
			else error("Argumentname erwartet");
			}
		undeclared--;	/* cnt down  jrvz 8/6/82 */
		if(endst())return;
		if(match(",")==0)error("Komma erwartet");
		}
	}
/*					*/
/*	Statement parser		*/
...
ns()	{if(match(";")==0)error("Fehlendes Semikolon");}
/*					*/
/*	Compound statement		*/
/*					*/
...
illname()
{	error("Unzulaessiger Symbolname");junk();}
multidef(sname)
	char *sname;
{	error("Bereits definiert");
	comment();
	outstr(sname);nl();
}
needbrack(str)
	char *str;
{	if (match(str)==0)
		{error("Fehlende Klammer");
		comment();outstr(str);nl();
		}
}
needlval()
{	error("Muss ein LVALUE sein");
}
hash(sname)
	char *sname;
{	int h,c;
...
{	char *ptr;
	...
	if(glbptr>=ENDGLB)
		{error("Ueberlauf der globalen Symboltabelle");
		return 0;
		}
	ptr=cptr;
...
{	char *ptr;
	...
	if(locptr>=ENDLOC)
		{error("Ueberlauf der lokalen Symboltabelle");
		return 0;
		}
	cptr=ptr=locptr;
	while(an(*ptr++ = *sname++));	/* copy name */
...
addwhile(ptr)
	int ptr[];
 {
	int k;
	if (wqptr==wqmax)
		{error("Zu viele WHILEs geoeffnet");return;}
	k=0;
	while (k<wqsiz)
		{*wqptr++ = ptr[k++];}
}
...
readwhile()
{	if (wqptr==wq){error("Kein offenes WHILE");return 0;}
...
{	int k;
	...
	while(ch())
		{if((ch()==' ')|(ch()==9))
			...
			((line[lptr-1]==92)&(line[lptr-2]!=92))
				...
				{if(ch()==0)
				   {error("Fehlendes Anfuehrungszeichen");
				   break;
				   }
				keepch(gch());
				}
	...
	keepch(0);
	if(mptr>=mpmax)error("Zeile zu lang");
	lptr=mptr=0;
...
{	char sname[namesize];
	...
	while(ch()==' ' | ch()==9) gch();
	while(putmac(gch()));
	if(macptr>=macmax)error("Makrotabelle voll");
	}
putmac(c)
	char c;
{	macq[macptr]=c;
	...
	char c;
{
	if(c)	/* sense of test reversed  jrvz 9/2/83 */
		{if(output)
			{if((putc(c,output))<=0)
				{closeout();
				error("Fehler beim Schreiben in die Datei");
				abort();	/* gtf 7/17/80 */
				}
			}
		else putchar(c);
...
{	int k;
	...
	toconsole();
	bell();
	outstr("Zeile "); outdec(lineno); outstr(", ");
	if(infunc==0)
		outbyte('(');
	if(currfn==NULL)
		outstr("Dateianfang");
