/*	name...
	...
	history...
                23 Mar 87 -mk- corrected avail() error;
                          JP CCSUB does not work instead of CALL CCSUB and RET
			  because of stack manipulations in CCSUB
...
QAVAIL:	LD	HL,(6)	;end of TPA
	...
	CALL    CCSUB	;find (6)-HEAPTOP
        RET             ; don't use JP CCSUB instead of CALL CCSUB and RET
;
...
{	char c,*fcb;
	...
	if (name[1]==':')		/* transfer disk */
		{fcb[0]= (*name&15);    /* either case --- DELETED "+1" by MK*/
		name=name+2;
		}
	else	fcb[0]=_dfltdsk;
