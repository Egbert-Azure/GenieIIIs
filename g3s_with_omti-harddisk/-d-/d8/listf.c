...
extern FILE *printer;            /* pointer to the printer */
...
open_print_file()
{
   printer = fopen("LST:","w");       /* open printer file */
}
...
int index;
   ...
   if (strlen(oneline) < 72)
      fprintf(printer,"%5d %s",line_number,oneline);

                          /* This prints a line of 72 to 143 chars */
   else if (strlen(oneline) < 144) {
      fprintf(printer,"%5d ",line_number);
...
int index;
   ...
   if (lines_this_page == 0) {        /* top of every printer page */
      fprintf(printer,"\n\n\n        ");
      fprintf(printer," Source file - %s        ",filename);
